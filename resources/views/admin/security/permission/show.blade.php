<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <x-secondary-button type="button" x-data="{route: '{{ route('admin.permissions.index') }}'}" x-on:click="window.location.href=route">
                ...
                <div class="max-w-xl">
                    ...
                    <div class="mt-6 space-y-6">
                        <div>
                            ...
                            <x-text-input disabled id="name" name="name" value="{{ $permission->name }}" class="mt-1 block w-full" />
                        </div>
                    </div>
                </div>
            ...
            @can('editar.permissoes')
                <x-primary-button
                    type="button"
                    x-data="{route: '{{ route('admin.permissions.edit', ['permission' => $permission->id]) }}'}"
    ...
    @can('excluir.permissoes')
        <x-modal name="confirm-user-deletion" :show="$errors->isNotEmpty()" focusable>
            <form action="{{ route('admin.permissions.destroy', ['permission' => $permission->id]) }}" method="post" class="p-6">
                @csrf
                @method('DELETE')
                ...
                <div class="mt-6 flex justify-end">
                    ...
                    <x-danger-button class="ml-3">
                        <i class="fa-solid fa-trash"></i>&nbsp;{{ __('Sim, excluir esta permissão.') }}
                    </x-danger-button>
                </div>
            </form>
        </x-modal>
